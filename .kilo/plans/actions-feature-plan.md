...
- [x] (2026-08-31) Precompute icon-pack values in Settings to remove tab-switch jank. Moved `queryIntentActivities()` from Composable lambda into `onCreateView` before `setContent`, then further optimized by computing on `Dispatchers.IO` and precomputing icon-pack display names into a map to avoid repeated `loadLabel()` Binder calls during recomposition.
...
- Observation: Settings icon-pack summary recomposition triggers repeated `PackageManager.loadLabel()` Binder calls.
  Evidence: `SettingsFragment.iconPackName()` called `HPackages.getApplicationInfoOrNull(pack)?.loadLabel(app.packageManager)` on every recomposition of the `ListPreference` summary. Each call is a Binder IPC. With 20+ preference items and Compose's recomposition behavior, this created additional main-thread jank even after moving `queryIntentActivities()` out of composition. Fixed by precomputing a `packageName -> displayName` map on `Dispatchers.IO` and reading from state during composition.
