...
function isAllowedWebSocketOrigin(req) {
  ...
  // Accept both http:// and https:// origins matching the host
...
function maybeOpenBrowser() {
  ...
  // Operator-provided launcher: split into executable + args (no shell),
  // so a command containing shell metacharacters can't inject.
  if (process.env.BRAINSTORM_OPEN_CMD) {
    const parts = process.env.BRAINSTORM_OPEN_CMD.trim().split(/\s+/);
    if (parts.length > 0) {
      const bin = parts[0];
      const args = parts.slice(1).concat(JSON.stringify(url));
