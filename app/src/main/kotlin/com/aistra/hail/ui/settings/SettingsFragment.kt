...
class SettingsFragment : MainFragment(), MenuProvider {
    ...
    private var backupLauncher = registerForActivityResult(ActivityResultContracts.CreateDocument("application/zip")) { uri ->
    ...
    private var restoreLauncher = registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        if (uri == null) return@registerForActivityResult
        val cacheDir = context?.cacheDir ?: return@registerForActivityResult
        val ctx = context ?: return@registerForActivityResult
        lifecycleScope.launch {
    ...
    private fun SettingsScreen() {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize()) {
            ...
            horizontalDivider()
            preferenceCategory(key = "backup_category", title = { Text(text = stringResource(R.string.title_backup)) })
            preference(
                key = "backup",
