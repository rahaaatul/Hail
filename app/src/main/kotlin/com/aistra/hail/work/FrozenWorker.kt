...
class FrozenWorker(context: Context, params: WorkerParameters) : Worker(context, params) {
    override fun doWork(): Result {
        ...
        return if (AppManager.setAppFrozen(packageName, shouldFreeze)) {
            ...
            // Failed freeze is almost always permanent (wrong working mode,
            // package not found/disabled). Failed unfreeze may be transient
            // (temporary binder/permission issues), so retry.
